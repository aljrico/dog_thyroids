...
thyroids_unique_value <- function(){
  clean_thyroids %>% 
    group_by(id) %>% 
    summarise(
...
braqui_atenuation_test <- function(){
  clean_thyroids %>% 
    group_by(id) %>% 
    summarise(
      right_NC = mean(right_NC_avg, na.rm = TRUE),
...
clean_thyroids <- get_clean_thyroids()

blood_thyroids_corr <- function(){
